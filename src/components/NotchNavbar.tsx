...
function RailHairlines() {
  return (
    <svg className="pointer-events-none absolute inset-0 h-full w-full" preserveAspectRatio="none" aria-hidden="true">
      <line x1="0" y1="51.5" x2="100%" y2="51.5" stroke="white" strokeOpacity={0.14} strokeWidth={0.5} />
      <line x1="0" y1="48.5" x2="100%" y2="48.5" stroke="white" strokeOpacity={0.14} strokeWidth={0.5} />
    </svg>
  );
}
  ...
  return (
    <>
      <header className={cn("fixed inset-x-0 top-0 z-40 flex h-16 px-0", className)}>
        ...
        <div className="relative z-20 h-[52px] min-w-8 flex-1 bg-notch">
          <RailHairlines />
          <div className="pointer-events-none absolute inset-x-0 top-full h-8 bg-gradient-to-b from-notch/70 to-transparent" aria-hidden="true" />
        </div>
        ...
        <div className="relative z-10 -ml-[2px] flex h-16 shrink-0 shadow-[0_14px_36px_rgba(8,25,48,0.35)]">
          ...
          <div className="relative h-full w-[50px] shrink-0">
            <div className="absolute inset-0 bg-notch" style={{ clipPath: "path('M0 0 H50 V64 C25 64 25 52 0 52 Z')" }} />
            <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 50 64" aria-hidden="true">
              <path d="M0 51.5 C25 51.5 25 63.5 50 63.5" fill="none" stroke="white" strokeOpacity={0.14} strokeWidth={0.5} />
              <path d="M0 48.5 C25 48.5 25 60.5 50 60.5" fill="none" stroke="white" strokeOpacity={0.14} strokeWidth={0.5} />
            </svg>
          </div>
          ...
          <div className="relative -ml-[2px] min-w-0 flex-1">
            ...
            <div className="relative flex h-full w-full items-end justify-between gap-3 px-4 pb-2 md:px-6">
              <nav className="mb-1 hidden shrink-0 items-center gap-5 min-[1400px]:flex" aria-label="Main navigation">
                {left.map((entry) => (
              ...
              <button
                type="button"
                className="mb-1 p-1 text-white/70 transition-colors hover:text-white min-[1400px]:hidden"
              ...
              <nav className="mb-1 hidden shrink-0 items-center gap-5 min-[1400px]:flex" aria-label="Secondary navigation">
                {right.map((entry) => (
                  <NotchLink key={entry.id} entry={entry} active={entry.id === activeId} onClick={() => go(entry.id)} />
                ))}
              </nav>
          ...
          <div className="relative -ml-[2px] h-full w-[50px] shrink-0">
            <div className="absolute inset-0 bg-notch" style={{ clipPath: "path('M0 0 H50 V52 C25 52 25 64 0 64 Z')" }} />
            <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 50 64" aria-hidden="true">
              <path d="M0 63.5 C25 63.5 25 51.5 50 51.5" fill="none" stroke="white" strokeOpacity={0.14} strokeWidth={0.5} />
              <path d="M0 60.5 C25 60.5 25 48.5 50 48.5" fill="none" stroke="white" strokeOpacity={0.14} strokeWidth={0.5} />
            </svg>
          </div>
        </div>

        {/* Right rail + ocean wash */}
        <div className="relative z-20 -ml-[2px] h-[52px] min-w-8 flex-1 bg-notch">
          <RailHairlines />
          <div className="pointer-events-none absolute inset-x-0 top-full h-8 bg-gradient-to-b from-notch/70 to-transparent" aria-hidden="true" />
        </div>
      </header>
      ...
      {open && (
        <nav
          className="fixed inset-x-3 top-[4.5rem] z-40 rounded-2xl border border-white/10 bg-notch p-4 shadow-xl min-[1400px]:hidden"
